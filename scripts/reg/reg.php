<?php
 ...
 if(isset($_GET['active']) and $_GET['active'] == 'ok')
	echo '<b>Ваш аккаунт CATOCA DATABASE успешно активирован!</b>';
 ...
 if(isset($_GET['key']))
 {
	...
	else
	{
		...
		$title = 'Ваш аккаунт CATOCA DATABASE успешно активирован';
		$message = 'Поздравляю Вас, Ваш аккаунт CATOCA DATABASE успешно активирован';
			
		sendMessageMail($email, CAT_MAIL_AUTOR, $title, $message);
			
		/*Перенаправляем пользователя на 
		нужную нам страницу*/
		header('Location:'. CAT_HOST .'?mode=reg&active=ok');
		exit;
	}
 }
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				$sql = 'INSERT INTO `'. CAT_DBPREFIX .'reg`
						VALUES(
								"",
								:email,
								:pass,
				...
				//Отправляем письмо для активации
				$url = CAT_HOST .'?mode=reg&key='. md5($salt);
				$title = 'Регистрация CATOCA DATABASE';
				$message = 'Для активации Вашего акаунта пройдите по ссылке 
				<a href="'. $url .'">'. $url .'</a>';
				
				sendMessageMail($_POST['email'], CAT_MAIL_AUTOR, $title, $message);
				
				//Сбрасываем параметры
				header('Location:'. CAT_HOST .'?mode=reg&status=ok');
				exit;
			}
		}
	}
